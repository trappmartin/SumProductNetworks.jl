...
llhtest = mean([llh(root, X[:,i])[1] for i in 1:N])
println("* initial llh: ", llhtest)
...
@time for observation in 1:1
	...
	(config, cMax) = SPN.processIterative(spn, assign, X, observation)
	...
	(spn, assign) = SPN.updatePartitions!(X, spn, assign)
